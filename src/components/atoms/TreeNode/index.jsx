			...
			>
				...
				>
					...
					{color !== preColor ? (
						<span
							className={classNames(
								`absolute h-0.5 w-4 top-1/2 translate-x-1/2 rounded-l-none z-0 transition duration-300`,
								isParentHovered
									? `bg-${color}-400 -left-[21px] rounded-b-full`
									: `bg-${color}-200 -left-[19px]`
							)}
						/>
					) : null}
					<Link
				...
				{node.children?.length ? (
					...
					>
						<div className="pl-6 relative">
							<span
								className={classNames(
									"absolute left-4 -top-4 bottom-[18px] z-10 translate-x-[3px] rounded-l-full w-0.5 transition duration-300",
									isParentHovered
										? `bg-${preColor}-400`
										: `bg-${preColor}-200`
								)}
